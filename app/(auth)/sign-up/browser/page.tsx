...
import { signUpBrowser, type AuthActionState } from "@/lib/auth/actions";

const initialState: AuthActionState = {};

export default function BrowserSignUpPage() {
  const [state, formAction, pending] = useActionState(signUpBrowser, initialState);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-20 text-center">
      ...
      <h1 className="mb-8 font-serif text-3xl text-gold-100">Free. Everything unlocked.</h1>
      ...
      <form action={formAction} className="flex flex-col gap-4 text-left">
        ...
        >
          {pending ? "Creating account…" : "Create Free Account & Start Browsing →"}
        </button>
      </form>
